<template>
    <Head>
        <title>Tambah Soal Ujian - Buweuk Sipit Academy</title>
    </Head>
        ...
        <div class="row">
            <div class="col-md-12">
                <Link
                    ...
                    Kembali ke Detail Ujian</Link
                >

                <!-- Banner: ujian baru dibuat -->
                <div v-if="just_created" class="alert alert-success border-0 shadow-sm mb-3 d-flex align-items-center gap-3">
                    <i class="fa fa-check-circle fa-2x text-success"></i>
                    <div>
                        <strong>Ujian "{{ exam.title }}" berhasil dibuat!</strong><br/>
                        <span class="text-muted">Sekarang tambahkan soal untuk ujian ini.</span>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow">
                    <div class="card-body">
                        <h5>
                            <i class="fa fa-question-circle"></i> Tambah Soal Ujian
                        </h5>
                        <h6 v-if="isPersonality" class="text-muted">
                            <i class="fa fa-info-circle me-1"></i> Tipe:
                            Kepribadian - Point default: A=5, B=4, C=3, D=2, E=1
                        </h6>
                        <hr />
                        <form @submit.prevent>
                                ...
                                <table class="table-bordered table-centered table-nowrap mb-0 table rounded">
                                    <tbody>
                                        ...
                                        <tr v-if="isPersonality">
                                            ...
                                            <td>
                                                <div class="row">
                                                    <div class="col-md-2"><label>A</label><input type="number" class="form-control" v-model="form.point_1" min="1" max="5" /></div>
                                                    <div class="col-md-2"><label>B</label><input type="number" class="form-control" v-model="form.point_2" min="1" max="5" /></div>
                                                    <div class="col-md-2"><label>C</label><input type="number" class="form-control" v-model="form.point_3" min="1" max="5" /></div>
                                                    <div class="col-md-2"><label>D</label><input type="number" class="form-control" v-model="form.point_4" min="1" max="5" /></div>
                                                    <div class="col-md-2"><label>E</label><input type="number" class="form-control" v-model="form.point_5" min="1" max="5" /></div>
                                                </div>
                            ...
                            <div class="d-flex gap-2 flex-wrap">
                                ...
                                >
                                    <i class="fa fa-plus-circle me-1"></i> Simpan &amp; Tambah Lagi
                                </button>
                                <button
                                    type="button"
                                    class="btn btn-md btn-success border-0 shadow"
                                    @click="submitAndFinish"
                                >
                                    <i class="fa fa-check me-1"></i> Simpan &amp; Selesai
                                </button>
                                <Link
                                    :href="`/admin/exams/${exam.id}`"
                                    class="btn btn-md btn-secondary border-0 shadow"
                                >
                                    <i class="fa fa-times me-1"></i> Selesai (Tanpa Simpan)
                                </Link>
                            </div>
                        </form>
                    </div>
...
export default {
    ...
    setup(props) {
            ...
            router.post(
                ...
                buildData(),
                {
                    ...
                    onSuccess: () => {
                        ...
                        Swal.fire({
                            title: 'Berhasil!',
                            text: 'Soal disimpan. Silakan tambah soal berikutnya.',
                            icon: 'success',
                ...
                buildData(),
                {
                    onSuccess: () => {
                        Swal.fire({
                            ...
                            router.visit(`/admin/exams/${props.exam.id}`);
                        });
                    },
                },
            );
        ...
        return { form, isPersonality, editorConfig, submitAndAddMore, submitAndFinish };
    },
};
</script>
